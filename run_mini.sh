...
echo "0-Init"
cd `dirname $0`
docker compose -f docker-compose-run-mini.yml down --timeout 0 || true
#cp data-1 data -r
...
docker run --rm  -v "./build:/work" -w /work --network none ${IMAGE} bash build.sh
...
#docker run --cpuset-cpus="16-23" --cpus=8 -m 2G --rm -it -v ./testbench:/work -w /work --network alimama ${IMAGE} make
docker compose -f docker-compose-run-mini.yml up -d
#echo "4-Tail Log"
#tail -F node-testbench/logs/test.log
#docker run --cpuset-cpus="16-23" --cpus=8 -m 2G --rm -it -v ./testbench:/work -w /work --network alimama ${IMAGE} ./testbench ./data.json
